...
def train(model, epoch, args, device, train_loader, optimizer, masks = None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if time.time() - last_tick >= 0.2 or batch_idx == len(train_loader) - 1:
            ...
            print("\r{:3d} \033[1;32m{:5f} ({:.4f}) \033[1;37m({:3d}%) ({:s}, {:s})".format(epoch+1,total_loss / (batch_idx+1), 1 - (accuracy / ((1+batch_idx) * args.batch_size)), int(100 * (1+batch_idx) / len(train_loader)),duration(eta), duration(time.time() - start_epoch)),end='')
